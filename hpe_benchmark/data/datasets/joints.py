...
from hpe_benchmark.data.transforms import get_affine_transform
from hpe_benchmark.data.transforms import affine_transform
from hpe_benchmark.data.transforms import flip_joints
...
class JointsDataset(Dataset):
    def __init__(self, cfg, stage, transform=None):
        ...
        self.keypoint_num = cfg.KEYPOINT.NUM
        self.flip_pairs = cfg.KEYPOINT.FLIP_PAIRS
        self.upper_body_ids = cfg.KEYPOINT.UPPER_BODY_IDS
        self.lower_body_ids = cfg.KEYPOINT.LOWER_BODY_IDS
        self.kp_load_min_num = cfg.KEYPOINT.LOAD_MIN_NUM

        self.input_shape = cfg.DATA.INPUT_SHAPE
        self.output_shape = cfg.DATA.OUTPUT_SHAPE
        self.w_h_ratio = self.input_shape[1] / self.input_shape[0]

        self.pixel_std = cfg.DATA.PIXEL_STD
        self.color_rgb = cfg.DATA.COLOR_RGB

        self.basic_ext = cfg.DATA.BASIC_EXTENTION
        self.rand_ext = cfg.DATA.RANDOM_EXTENTION
        self.x_ext = cfg.DATA.X_EXTENTION
        self.y_ext = cfg.DATA.Y_EXTENTION
        self.scale_factor_low = cfg.DATA.SCALE_FACTOR_LOW
        self.scale_factor_high = cfg.DATA.SCALE_FACTOR_HIGH
        self.scale_shrink_ratio = cfg.DATA.SCALE_SHRINK_RATIO
        self.rotation_factor = cfg.DATA.ROTATION_FACTOR
        self.prob_rotation = cfg.DATA.PROB_ROTATION
        self.prob_flip = cfg.DATA.PROB_FLIP
        self.num_keypoints_half_body = cfg.DATA.NUM_KEYPOINTS_HALF_BODY
        self.prob_half_body = cfg.DATA.PROB_HALF_BODY
        self.x_ext_half_body = cfg.DATA.X_EXTENTION_HALF_BODY
        self.y_ext_half_body = cfg.DATA.Y_EXTENTION_HALF_BODY
        self.add_more_aug = cfg.DATA.ADD_MORE_AUG
        self.gaussian_kernels = cfg.DATA.GAUSSIAN_KERNELS

        self.test_x_ext = cfg.TEST.X_EXTENTION
        self.test_y_ext = cfg.TEST.Y_EXTENTION
    ...
    def __getitem__(self, idx):
        ...
        trans = get_affine_transform(
            center, scale, rotation, self.input_shape)
        img = cv2.warpAffine(
            data_numpy,
            trans,
    ...
    def half_body_transform(self, joints, joints_vis):
        ...
        # TODO: randn -> rand
        if np.random.rand() < 0.5 and len(upper_joints) > 3:
            selected_joints = upper_joints
        else:
            selected_joints = lower_joints if len(
                lower_joints) > 3 else upper_joints

        if len(selected_joints) < 3:
            return None, None

        selected_joints = np.array(selected_joints, dtype=np.float32)
        # find the property along specific axis
        ...
        scale = np.array([w * 1.0 / self.pixel_std, h *
                          1.0 / self.pixel_std], dtype=np.float32)
