...
from hpe_benchmark.layers.jointsmseloss import JointsMSELoss
...
class PResNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.inplanes = 64
        self.deconv_with_bias = cfg.MODEL.RESNET.DECONV_WITH_BIAS
        ...
        self.deconv_layers = self._make_deconv_layer(
            cfg.MODEL.RESNET.NUM_DECONV_LAYERS,
            cfg.MODEL.RESNET.NUM_DECONV_FILTERS,
            cfg.MODEL.RESNET.NUM_DECONV_KERNELS,
        )

        self.final_layer = nn.Conv2d(
            in_channels=cfg.MODEL.RESNET.NUM_DECONV_FILTERS[-1],
            out_channels=cfg.KEYPOINT.NUM,
            kernel_size=cfg.MODEL.RESNET.FINAL_CONV_KERNEL,
            stride=1,
            padding=1 if cfg.MODEL.RESNET.FINAL_CONV_KERNEL == 3 else 0
        )
    ...
    def _calculate_loss(self, outputs, valids, labels):
        target_weights = valids.squeeze(-1)
        target = labels[:, 1, :, :, :]  # corresponding to gaussian kernel (11, 11)
        criterion = JointsMSELoss()
        loss = criterion(outputs, labels, valids)
